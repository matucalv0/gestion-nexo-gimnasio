...
WHERE (dni, fecha_hora) NOT IN (
    ...
    GROUP BY dni, CAST(fecha_hora AT TIME ZONE 'America/Argentina/Buenos_Aires' AS DATE)
);

-- Paso 2: crear el índice único para blindar futuros inserts.
CREATE UNIQUE INDEX IF NOT EXISTS uq_asistencia_dni_fecha
    ON public.asistencia (dni, CAST(fecha_hora AT TIME ZONE 'America/Argentina/Buenos_Aires' AS DATE));
