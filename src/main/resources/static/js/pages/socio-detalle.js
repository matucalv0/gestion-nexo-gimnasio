...
async function cargarMembresiaVigente(dni) {
    ...
    try {
        ...
        /* ❗ Membresía vencida (backend manda message) */
        if (res.status === 409) {
        ...
        container.innerHTML = `
    <div class="bg-[#16232c] rounded-lg p-4 space-y-1">
        <p class="text-xs uppercase tracking-wide text-gray-400">Nombre</p>
        <p class="text-lg font-semibold">${m.tipo}</p>
    </div>

    <div class="bg-[#16232c] rounded-lg p-4 space-y-1">
        <p class="text-xs uppercase tracking-wide text-gray-400">Tipo</p>
        <p class="text-lg font-semibold">${m.tipoMembresia}</p>
    </div>

    <div class="bg-[#16232c] rounded-lg p-4 space-y-2">
        <p class="text-xs uppercase tracking-wide text-gray-400">Estado</p>
        <span class="inline-flex items-center gap-2 bg-green-600/20 text-green-400 px-3 py-1 rounded-full text-sm font-medium">
    ...
    <div class="bg-[#16232c] rounded-lg p-4 space-y-1">
        <p class="text-xs uppercase tracking-wide text-gray-400">Vencimiento</p>
